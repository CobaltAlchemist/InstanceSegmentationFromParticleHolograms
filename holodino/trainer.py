...
import weakref
import holodino
from holodino.dataset import HolodinoInstanceDatasetMapper, HolodinoInstanceGenerativeDatasetMapper, HolodinoInstanceGenerativeDataset
from holodino.scheduler import CosineAnnealingWithWarmRestartScheduler
...
class Trainer(DefaultTrainer):
    ...
    def __init__(self, cfg):
        ...
        logger.info(model)
        logger.info("Trainable parameters: " + str(sum(p.numel() for p in model.parameters() if p.requires_grad)))
        ...
        def print_params(module, indent=0):
            output = ' ' * indent + str(type(module).__name__) + ": " + human_format(count_parameters(module)) + " parameters\n"
            for name, child in module.named_children():
                output += print_params(child, indent + 2)
            return output
        
        def model_to_dict(module):
            module_dict = {
                "type": type(module).__name__,
                "parameters": human_format(count_parameters(module)),
                "children": {}
            }
    
            for name, child in module.named_children():
                module_dict["children"][name] = model_to_dict(child)
            
            return module_dict
        from torch.utils.tensorboard import SummaryWriter
        import json

        with SummaryWriter(cfg.OUTPUT_DIR) as w:
            w.add_text('params', json.dumps(model_to_dict(model), indent=2))
    ...
    def build_train_loader(cls, cfg):
        ...
        # Semantic segmentation dataset mapper
            ...
            return build_detection_train_loader(cfg, mapper=mapper)
        elif cfg.INPUT.DATASET_MAPPER_NAME == "holodino_instance":
            mapper = HolodinoInstanceDatasetMapper(cfg, True)
            return build_detection_train_loader(cfg, mapper=mapper)
        elif cfg.INPUT.DATASET_MAPPER_NAME == "holodino_autogen":
    ...
    def build_lr_scheduler(cls, cfg, optimizer):
        ...
        if name == "CosineAnnealingWithWarmRestart":
            end_value = cfg.SOLVER.BASE_LR_END / cfg.SOLVER.BASE_LR
            assert end_value >= 0.0 and end_value <= 1.0, end_value
            assert cfg.SOLVER.ANNEALING_CYCLES > 0
            sched = CosineAnnealingWithWarmRestartScheduler(
                1, end_value, cfg.SOLVER.ANNEALING_CYCLES
            )
    ...
    def build_optimizer(cls, cfg, model):
        ...
        def add_gradient_accumulation(optim):
            accumulation_steps = cfg.SOLVER.GRADIENT_ACCUMULATION_STEPS
            ...
            class GradientAccumulationOptimizer(optim):
                ...
                def step(self, closure=None):
                    if (self.step_count + 1) % accumulation_steps == 0:
                        super().step(closure=closure)
                ...
                def zero_grad(self):
                    if (self.step_count + 1) % accumulation_steps == 0:
                        super().zero_grad()
                    self.step_count += 1
            return GradientAccumulationOptimizer

        optimizer_type = cfg.SOLVER.OPTIMIZER
        if optimizer_type == "SGD":
            opt_type = torch.optim.SGD
            opt_type = maybe_add_full_model_gradient_clipping(opt_type)
            opt_type = add_gradient_accumulation(opt_type)
            optimizer = opt_type(params, cfg.SOLVER.BASE_LR, momentum=cfg.SOLVER.MOMENTUM)
        elif optimizer_type == "ADAMW":
            opt_type = torch.optim.AdamW
            opt_type = maybe_add_full_model_gradient_clipping(opt_type)
            opt_type = add_gradient_accumulation(opt_type)
            optimizer = opt_type(params, cfg.SOLVER.BASE_LR)
        else:
